import { incomeTaxServices } from '@/app/data/services/income-tax';
...
export default async function IncomeTaxServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const service = incomeTaxServices.find((s) => s.slug === slug);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <ScrollReveal direction="up">
          <div className="max-w-4xl mx-auto mb-16">
            ...
            <div className="bg-success/5 border-l-4 border-success rounded-r-lg p-6">
              <p className="text-gray-700">
                <strong className="text-primary">CA Verified:</strong> All our tax returns are prepared and reviewed by experienced Chartered Accountants, ensuring 100% accuracy and compliance with latest IT laws.
              </p>
            </div>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal direction="up">
          <div className="mb-16">
            <div className="bg-gradient-to-br from-primary/5 to-success/5 rounded-2xl p-8">
        ...
        <ScrollReveal direction="up">
          <RelatedServices
            services={incomeTaxServices.map(({ icon, ...s }) => ({ ...s, iconName: s.iconName ?? 'FileText' }))}
