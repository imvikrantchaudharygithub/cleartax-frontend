import { trademarkServices } from '@/app/data/services/trademarks';
...
export default async function TrademarkServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const service = trademarkServices.find((s) => s.slug === slug);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <ScrollReveal direction="up">
          <div className="max-w-4xl mx-auto mb-16">
            ...
            <div className="bg-warning/5 border-l-4 border-warning rounded-r-lg p-6">
              <p className="text-gray-700">
                <strong className="text-primary">Legal Protection:</strong> Our experienced IP attorneys ensure your intellectual property is properly protected with comprehensive legal documentation and representation.
              </p>
            </div>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal direction="up">
          <RelatedServices
            services={trademarkServices.map(({ icon, ...s }) => ({ ...s, iconName: s.iconName ?? 'FileText' }))}
